package com.escalade.servlets;

import java.io.IOException;
import java.util.HashMap;
import java.util.Map;

import javax.servlet.ServletException;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import javax.servlet.http.HttpSession;

import com.escalade.beans.Pret;
import com.escalade.beans.Topo;
import com.escalade.dao.DAOFactory;
import com.escalade.dao.PretDao;
import com.escalade.dao.TopoDao;
import com.escalade.forms.CreationPretForm;

public class CreationPret extends HttpServlet {
	
	 public static final String CONF_DAO_FACTORY = "daofactory";
	    public static final String ATT_PRET= "pret";
	    public static final String ATT_FORM = "form";
	    public static final String SESSION_TOPOS= "topos";
	    public static final String SESSION_PRETS= "prets";
	    public static final String APPLICATION_TOPOS = "initTopos";
	   
	    public static final String VUE_SUCCES= "/WEB-INF/afficherPret.jsp";
	    public static final String VUE_FORM = "/WEB-INF/creerPret.jsp";

	    private TopoDao  topoDao;
	    private PretDao pretDao;

	    public void init() throws ServletException {
	      
	        this.topoDao = ( (DAOFactory) getServletContext().getAttribute( CONF_DAO_FACTORY ) ).getTopoDao();
	        this.pretDao = ( (DAOFactory) getServletContext().getAttribute( CONF_DAO_FACTORY ) ).getPretDao();
	    }

	    public void doGet( HttpServletRequest request, HttpServletResponse response ) throws ServletException, IOException {
	       
	        this.getServletContext().getRequestDispatcher( VUE_FORM ).forward( request, response );
	    }

	    public void doPost( HttpServletRequest request, HttpServletResponse response ) throws ServletException, IOException {
	   
	        CreationPretForm form = new CreationPretForm( topoDao, pretDao );

	        Pret pret = form.createPret( request );

	       
	        request.setAttribute( ATT_PRET, pret );
	        request.setAttribute( ATT_FORM, form );

	       
	        if ( form.getErreurs().isEmpty() ) {
	           
	            HttpSession session = request.getSession();
	            Map<Long, Topo> topos = (HashMap<Long, Topo>) session.getAttribute( SESSION_TOPOS );
	           
	            if ( topos == null ) {
	                topos= new HashMap<Long, Topo>();
	            }
	          
	            topos.put( pret.getTopo().getId(), pret.getTopo() );
	          
	            session.setAttribute( SESSION_TOPOS, topos);
	            Map<Long, Pret> prets = (HashMap<Long, Pret>) session.getAttribute( SESSION_PRETS );
	            
	            if ( prets == null ) {
	                prets = new HashMap<Long, Pret>();
	            }
	          
	            prets.put( pret.getId(), pret);
	            
	            session.setAttribute( SESSION_PRETS, prets);

	           
	            this.getServletContext().getRequestDispatcher( VUE_SUCCES ).forward( request, response );
	        } else {
	           
	            this.getServletContext().getRequestDispatcher( VUE_FORM ).forward( request, response );
	        }
	    }

}
